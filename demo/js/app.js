...
function viewHome() {
  ...
  const heroDevice = bannerProduct?.device || 'iphone';
  const heroColor = bannerProduct?.colors?.[0]?.hex || '#D2653A';
  ...
  <section class="hero">
    ...
    <div class="wrap hero__grid">
      ...
      <div class="stage">
        ...
        <div class="stage__float">${deviceSVG(heroDevice, heroColor, { ariaLabel: bannerProduct?.name || 'iPhone' })}</div>
      </div>
    </div>
  </section>
    ...
    <div class="grid">
      ${cats.map(c => {
        ...
        <a class="cat" href="#/c/${esc(c.id)}">
          ...
          ${sample ? deviceSVG(sample.device, sample.colors?.[0]?.hex || '#7D7E80') : ''}
        </a>`;
      }).join('')}
    </div>
  </section>
...
function viewCategory(catId) {
  ...
  <section class="section wrap">
    ...
    <div class="grid mt-6" id="catGrid">
      ${items.length ? items.map(productCard).join('') : `<p class="dim">${esc(u('nothingHere'))}</p>`}
    </div>
  </section>`;
}
...
function render() {
  if (!document.startViewTransition) return paint();
...
function wireView(r) {
  ...
  $$('[data-swatch]').forEach(b => b.addEventListener('click', e => {
    e.stopPropagation();
...
function openBooking(s) {
  ...
  const pick = (sel, attr) => $$(sel + ' .chip').forEach(b => b.addEventListener('click', () => {
...
function renderCart() {
  ...
  foot.innerHTML = `
    ...
    <button class="btn btn--primary btn--block btn--lg">${esc(u('checkout'))}</button>`;
...
function renderFooter() {
  ...
  $('#footGrid').innerHTML = `
    <div class="foot__col">
      <a href="#/" class="logo" style="margin-bottom:var(--sp-4)">
        <svg viewBox="0 0 40 40" fill="none" aria-hidden="true">
          <circle cx="20" cy="16.5" r="9.5" stroke="var(--accent)" stroke-width="2.6" stroke-linecap="round"/>
          <path d="M5 29.5h30" stroke="currentColor" stroke-width="2.8" stroke-linecap="round"/>
          <path d="M12 34.5h16" stroke="currentColor" stroke-width="2.8" stroke-linecap="round" opacity=".35"/>
        </svg>
        <span class="logo__word">i<b>Land</b></span>
      </a>
    ...
    <div class="foot__col">
      ...
      <a href="#/">${esc(u('shipping'))}</a>
      <a href="#/">${esc(u('returns'))}</a>
      <a href="#/">${esc(u('terms'))}</a>
      <a href="#/">${esc(u('privacy'))}</a>
    </div>
    <div class="foot__col">
      <h4>${esc(u('footVisit'))}</h4>
      ${s.phones.map(ph => `<a href="tel:${esc(ph.replace(/[^\d+]/g, ''))}">${esc(ph)}</a>`).join('')}
