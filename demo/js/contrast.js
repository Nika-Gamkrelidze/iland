/* ==========================================================================
   NORVA — adaptive text contrast, per glyph region
...
export function createContrast(fluid) {
  ...
  function rebuildMaps() {
    ...
    /* Secondary text keeps its reduced weight in BOTH states — mapping it to
       ...
       the moment a plume passes. --text-2 on dark ground, --ink-2 on bright. */
    dimUrl = paintMap(dimCnv, sample, [179, 169, 159], [75, 68, 61]);

    /* Accent: the lit brass on dark ground, the deep one on bright ground.
       Read from the tokens so a palette change carries through here too. */
    const accentLit = parseColor(cssColor('--brass', '#E3AE55'));
    const accentDeep = parseColor(cssColor('--brass-900', '#4E3208'));
    accentUrl = paintMap(accentCnv, sample, accentLit, accentDeep);
    return true;
  }
