...
function compile(gl, type, source, defines = '') {
  ...
  if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
    console.warn('fluid: shader compile failed\n', gl.getShaderInfoLog(sh));
    return null;
  }
  return sh;
}
...
function program(gl, vs, fs) {
  ...
  if (!gl.getProgramParameter(p, gl.LINK_STATUS)) {
    console.warn('fluid: program link failed\n', gl.getProgramInfoLog(p));
    return null;
  }
  const uniforms = {};
...
export function createFluid(canvas, opts = {}) {
  ...
  const palette = (opts.palette || ['#E3AE55', '#8E9BEA', '#F2D49A']).map(hexToRgb);
  let ground = hexToRgb(opts.ground || '#131110');
  let lightMode = false;
  ...
  /* These used to be hardcoded brand hexes that `opts.palette` did not
     override, so restyling the site left every splash the old colour. */
  const CORE = palette[0];
  const EDGE = palette[1] || palette[0];

  /** Centre of the plume is the primary; the edges drift to the secondary. */
  function plumeColor(s, spread) {
    const k = Math.min(1, Math.abs(s) * 0.55 + rand() * spread);
    const g = 0.5 + 0.34 * rand();
    return [
      (CORE[0] + (EDGE[0] - CORE[0]) * k) * g,
      (CORE[1] + (EDGE[1] - CORE[1]) * k) * g,
      (CORE[2] + (EDGE[2] - CORE[2]) * k) * g,
    ];
  }
