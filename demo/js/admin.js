/* ==========================================================================
   ...
   ("published numbers") operable — prices, instalment figures, repair costs,
   turnaround and stock are all edited in one place and shown exactly as the
   shopper will see them.

   Demo mode reaches exactly two controls in this file — catalogue import and
   photo upload — and nothing else: every ordinary CRUD path really writes.
   ========================================================================== */

import * as S from './store.js';
import { deviceSVG, DEVICE_KEYS } from './devices.js';
import { mountBanner, blockOn, demoBlocked, IS_DEMO, RESET_DONE } from './demo.js';
...
const A = {
  ...
  navService:   { ka: 'სახელოსნო',             en: 'Workshop',           ru: 'Мастерская' },
  ...
  fUpload:      { ka: 'ფოტოს არჩევა',          en: 'Choose photo',       ru: 'Выбрать фото' },
  ...
  condOpenBox:  { ka: 'გახსნილი შეფუთვა',      en: 'Open box',           ru: 'Открытая коробка' },
  badgeRefurb:  { ka: 'აღდგენილი',             en: 'Refurbished',        ru: 'Восстановлен' },
...
/* Blob URL for a photo the editor picked but nobody stored. Lives only as long
   as the slide-over that made it, so the tab never leaks one. */
let pendingPhoto = null;

function dropPendingPhoto() {
...
/* app.css ships four badge looks; `refurb` borrows the neutral one rather
   than inventing a class the storefront has no rule for. */
const BADGE_CLASS = { new: 'new', sale: 'sale', bestseller: 'best', refurb: 'refurb' };
...
function previewCard(p) {
  ...
  const badges = {
    ...
    sale:       '',   /* the struck price already says it */
    refurb:     `<span class="badge badge--demo">${esc(u('badgeRefurb'))}</span>`,
    bestseller: '<span class="badge badge--best">★</span>',
  };
  return `
  <article class="card">
    <div class="card__badges">${badges[p.badge] || ''}</div>
    <div class="card__media${p.image ? ' card__media--photo' : ''}">
      ${p.image
        ? `<img src="${esc(p.image)}" alt="${esc(p.name || '')}" class="product-photo" decoding="async">`
...
function viewDashboard() {
  ...
  /* Low and out are disjoint: a product sitting at zero is out, not low. */
  const low = st.products.filter(p => { const n = Number(p.stock) || 0; return n > 0 && n <= 5; });
  const out = st.products.filter(p => (Number(p.stock) || 0) <= 0);
  ...
  <section class="a-stats">
    ...
    <div class="a-stat${low.length || out.length ? ' a-stat--warn' : ''}">
      <b class="num">${low.length}</b>
      <span>${esc(u('statLow'))}</span>
      <small>${out.length} · ${esc(u('outOfStock'))}</small>
    </div>
  </section>
...
function productRows() {
  ...
  return rows.map(p => {
    ...
    <tr data-open="${esc(p.id)}">
      ...
      <td>${p.badge ? `<span class="badge badge--${esc(BADGE_CLASS[p.badge] || 'new')}">${esc(p.badge)}</span>` : '<span class="dim">—</span>'}</td>
      <td>
...
function blankProduct() {
  const cat = S.getState().categories[0]?.id || 'mac';
  return {
    id: '', sku: '', category: cat, device: 'laptop', name: '',
    tagline: { ka: '', en: '', ru: '' },
...
function openProductSheet(id) {
  ...
  <div class="a-group">
    ...
    <div class="a-row">
      ${sf('condition', u('fCondition'), draft.condition, [{ v: 'new', t: u('condNew') }, { v: 'open-box', t: u('condOpenBox') }])}
      ${sf('badge', u('fBadge'), draft.badge || '', [
        { v: '', t: u('none') }, { v: 'new', t: 'new' }, { v: 'sale', t: 'sale' },
        { v: 'bestseller', t: 'bestseller' }, { v: 'refurb', t: 'refurb' }])}
    ...
    <div class="a-row">
      ${tf('image', u('fImage'), draft.image || '', { attrs: 'placeholder="assets/products/mac-studio-m4-max.svg"' })}
    </div>
    <p class="a-hint">${esc(u('fImageHint'))}</p>
    <div class="flex gap-3 items-center wrap-flex">
      <button type="button" class="btn btn--ghost btn--sm" id="photoBtn">${icon('upload', 15)} ${esc(u('fUpload'))}</button>
      <input type="file" id="photoFile" accept="image/*" class="sr" aria-label="${esc(u('fUpload'))}">
      <span class="a-sub" id="photoName"></span>
    </div>
  ...
  const preview = form => {
    ...
    if (pendingPhoto) d.image = pendingPhoto;   /* preview only — never saved */
  ...
  openSheet({
    ...
    onMount(form) {
      /* A real file picker with a real local preview, and then the truth: this
         build has no object storage, so the picture goes nowhere. Blocking the
         click instead would hide what the control does in the product. */
      const photoFile = $('#photoFile', form);
      $('#photoBtn', form).addEventListener('click', () => photoFile.click());
      photoFile.addEventListener('change', () => {
        ...
        $('#photoName', form).textContent = file.name;
        refreshPreview();
        if (IS_DEMO) demoBlocked('upload');
      });
...
const TONES = [
  { v: 'orange', t: 'brass',  hex: 'var(--brass)' },
  { v: 'sky',    t: 'sky',    hex: '#7FB6E8' },
  { v: 'deep',   t: 'deep',   hex: 'var(--indigo)' },
  { v: 'sand',   t: 'chalk',  hex: 'var(--chalk-300)' },
];
...
function wireSettings() {
  ...
  $('#exportBtn').addEventListener('click', () => {
    ...
    a.href = url;
    a.download = `norva-cms-${today()}.json`;
  ...
  /* Import reads an arbitrary file and replaces the whole catalogue, so it is
     the one CMS control the public demo will not run. The button stays visible
     and enabled — the notice fires on click. Export, reset and every CRUD path
     below are untouched. */
  $('#importBtn').addEventListener('click',
    IS_DEMO ? blockOn('importJSON') : () => $('#importFile').click());

  $('#importFile').addEventListener('change', async e => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (IS_DEMO) { e.target.value = ''; demoBlocked('importJSON'); return; }
...
function applyLang() {
  ...
  demoBar?.repaint?.();   /* the banner is outside #adminMain, so repaint it here */
}
